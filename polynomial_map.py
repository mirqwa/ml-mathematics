...
def compute_parameters_and_predict(
    ...
    # computes the params using MAP
    phi_x = get_polynomial_input(X, degree)
    phi_x_transpose = np.transpose(phi_x)
    identity_matrix = np.identity(degree + 1)
    theta = np.dot(
        np.linalg.inv(np.dot(phi_x_transpose, phi_x) + 0.2**2 * identity_matrix), np.dot(phi_x_transpose, Y)
    )
...
def main() -> None:
    ...
    for degree in range(10):
        ...
        plot_data.plot_actual_and_predicted(
            ...
            Y,
            train_prediction,
            f"plots/polynomial_map_{degree}.png",
            f"MAP for polynomial with D={degree}",
        )
